...
if ! _has_yes_flag && [ -t 0 -o -e /dev/tty ]; then
  ...
  printf '%b' "${NC}"
  echo ""
  echo "  Zylos currently assumes a trusted environment. It runs"
  echo "  with full system access as the current user — it can"
  echo "  execute commands, read/write files, and access the network"
  echo "  on your behalf."
  echo ""
  printf '%b' "${YELLOW}"
  echo "  If untrusted people can reach this machine or talk to the"
  echo "  bot, they can execute anything. This is dangerous."
  printf '%b' "${NC}"
  echo ""
  printf '%b' "${DIM}"
  echo "  ──────────────────────────────────────────────────────────"
